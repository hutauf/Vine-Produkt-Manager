...
import { getPreferredTeilwert, getProductBaseValue, isProductIgnoredForBelegAndEuer } from '../../utils/euerUtils';
...
const EuerPage: React.FC<EuerPageProps> = ({ products, settings, onSettingsChange, additionalExpenses }) => {
  ...
  const euerData = useMemo(() => {
    ...
    products.forEach(p => {
      ...
      const baseValue = getProductBaseValue(p, settings) ?? 0;
      const entnahmeValue = getPreferredTeilwert(p) ?? baseValue;
      ...
      const isCancelled = p.usageStatus.includes(ProductUsage.STORNIERT);
      if (isCancelled) return;

      const isPrivatentnahmeOrDefault = p.usageStatus.includes(ProductUsage.PRIVATENTNAHME) || !p.usageStatus.length;
      const isPrivateOnlyModeApplicable = settings.privatentnahmeSameYearAsOnlyIncome &&
        p.usageStatus.includes(ProductUsage.PRIVATENTNAHME) &&
      ...
      if (isPrivateOnlyModeApplicable) {
        if (orderYear === selectedYear) {
          einnahmenProduktzugang += baseValue;
        }
        return;
      }
      ...
      if (settings.euerMethodETVInOutTeilwertEntnahme) {
        if (orderYear === selectedYear) {
          einnahmenProduktzugang += baseValue;
          ausgabenAnlagevermoegen += baseValue;
        }

        const noEntnahmeBecauseBusinessUsage =
          p.usageStatus.includes(ProductUsage.LAGER) || p.usageStatus.includes(ProductUsage.BETRIEBLICHE_NUTZUNG);

        if (isPrivatentnahmeOrDefault &&
            !noEntnahmeBecauseBusinessUsage &&
            privatentnahmeYear === selectedYear) {
          einnahmenPrivatentnahme += entnahmeValue;
        }
      } else {
        if (orderYear === selectedYear && (
        ...
        if (privatentnahmeYear === selectedYear && isPrivatentnahmeOrDefault) {
          einnahmenProduktzugang += baseValue;
        }
        ...
        if (orderYear === selectedYear) {
          if (p.usageStatus.includes(ProductUsage.BETRIEBLICHE_NUTZUNG)) {
            ...
            ausgabenUmlaufvermoegen += baseValue;
          }
        }
      }
    });
